...
#	python -m yamllint .
#	cython-lint ossis/compute/*.pyx
	python -m ruff check --fix --exit-zero
	python -m pycln .
	python -m isort .
	python -m ruff format ossis
...
test:
	...
	python -m coverage report --include=ossis/** --fail-under=60 -m

ci:
	python -m pip install --quiet --upgrade mypy
	python -m mypy --ignore-missing-imports --python-version 3.10 --no-strict-optional --check-untyped-defs ossis
