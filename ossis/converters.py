...
from ossis.compute.compiled import extract_columns_to_lists
...
from ossis.schema import convert_ossis_schema_to_arrow_schema
...
def to_arrow(dataset, size=None):
    ...
    if isinstance(dataset_schema, RelationSchema):
        try:
            arrow_schema = convert_ossis_schema_to_arrow_schema(dataset_schema)
        except MissingDependencyError:
            # Fall back to letting PyArrow infer types if optional dependency missing
            arrow_schema = None

        if arrow_schema is not None and any(
            col.type == ossisTypes.STRUCT and not getattr(col, "fields", None)
...
def from_arrow(tables, size=None):
    ...
    return rows_iterator, ossis_schema
