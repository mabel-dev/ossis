...
from ossis.group_by import GroupBy
from ossis.schema import RelationSchema
from ossis.tools import single_item_cache
from ossis.types import ossisTypes


class DataFrame:
    """
    ossis DataFrames are a lightweight container for tabular data.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Create an ossis DataFrame. DataFrames are a representation of a table with
        ...
        if isinstance(self._schema, RelationSchema):
            self._jsonb_columns = tuple(
                column.name for column in self._schema.columns if column.type == ossisTypes.JSONB
            )
        else:
            self._jsonb_columns = tuple()
        self.arraysize = 100
    ...
    def from_arrow(cls, tables):
        """Convert a PyArrow Table, or list of Tables, to an ossis DataFrame."""
    ...
    def arrow(self, size=None):
        """
        Convert an ossis DataFrame to a PyArrow Table, optionally limit the number
        of records.
        """
        from ossis.converters import to_arrow

        return to_arrow(self, size=size)

    def pandas(self, size=None):
        from ossis.converters import to_pandas

        return to_pandas(self, size)

    def polars(self, size=None):
        from ossis.converters import to_polars
    ...
    def append(self, entry):
        ...
        if isinstance(self._schema, RelationSchema):
            record: MutableMapping = entry if isinstance(entry, MutableMapping) else dict(entry)

            normalized_record = None
            orjson_module = None
            if self._jsonb_columns:
                for column_name in self._jsonb_columns:
                    ...
                    if isinstance(value, (dict, list, tuple, set)):
                        ...
                        if orjson_module is None:
                            ...
                            orjson_module = orjson
                        normalized_record[column_name] = orjson_module.dumps(value)
                if normalized_record is not None:
                    record = normalized_record

            # Ensure we work with a concrete dict when the input wasn't already one
    ...
    def select(self, attributes) -> "DataFrame":
        """
        Create a new ossis DataFrame with a subset of columns of an existing DataFrame
        """
