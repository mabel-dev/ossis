...
def test_from_arrow_with_single_table():
    ...
    expected_types = [ossisTypes.VARCHAR, ossisTypes.INTEGER]
    obj.materialize()
...
def test_opteryx_arrow_small():
    ...
    planets2 = ossis.DataFrame.from_arrow(planets_arrow)
    assert isinstance(planets2, ossis.DataFrame)
...
def test_opteryx_arrow_medium():
    ...
    fake2 = ossis.DataFrame.from_arrow(fake_arrow)
    assert isinstance(fake2, ossis.DataFrame)
