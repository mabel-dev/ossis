...
def test_get_ossis_type_parser_simple_types():
    """Test the get_ossis_type parser with simple type strings"""
...
def test_get_ossis_type_parser_complex_types():
    """Test the get_ossis_type parser with complex type strings"""
    assert get_ossis_type("ARRAY<INTEGER>") == ossisTypes.ARRAY
    assert get_ossis_type("ARRAY<VARCHAR>") == ossisTypes.ARRAY
    assert get_ossis_type("VARCHAR[255]") == ossisTypes.VARCHAR
    assert get_ossis_type("DECIMAL(10,2)") == ossisTypes.DECIMAL
    assert get_ossis_type("BLOB[1024]") == ossisTypes.BLOB


def test_get_ossis_type_parser_case_insensitive():
    """Test that get_ossis_type is case-insensitive"""
    assert get_ossis_type("integer") == ossisTypes.INTEGER
    assert get_ossis_type("INTEGER") == ossisTypes.INTEGER
    assert get_ossis_type("InTeGeR") == ossisTypes.INTEGER
    assert get_ossis_type("array<integer>") == ossisTypes.ARRAY
...
def test_get_ossis_type_parser_invalid_type():
    """Test that get_ossis_type raises ValueError for invalid types"""
    try:
