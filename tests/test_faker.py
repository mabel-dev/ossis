...
from ossis.schema import RelationSchema, FlatColumn, ColumnDisposition
from ossis.types import ossisTypes

from ossis.faker import generate_fake_data, generate_random_row
...
def test_generate_fake_data():
    ...
    columns = [
        ...
        FlatColumn(name="Name", type=ossisTypes.VARCHAR, disposition=ColumnDisposition.NAME),
        FlatColumn(name="Age", type=ossisTypes.INTEGER, disposition=ColumnDisposition.AGE),
    ]
    ...
    for row in df:
        ...
        for index, column in enumerate(columns):
            if column.type == ossisTypes.INTEGER:
                assert isinstance(row[index], int) or row[index] is None
            elif column.type == ossisTypes.VARCHAR:
...
def test_nullable_columns():
    ...
    columns = [
        FlatColumn(name="Nullable", type=ossisTypes.BOOLEAN, nullable=True),
        FlatColumn(name="NotNullable", type=ossisTypes.BOOLEAN, nullable=False),
    ]
...
def test_fakeable_types():
    ...
    columns = [
        FlatColumn(name="IntCol", type=ossisTypes.INTEGER),
        FlatColumn(name="VarCharCol", type=ossisTypes.VARCHAR),
        FlatColumn(name="BoolCol", type=ossisTypes.BOOLEAN),
        FlatColumn(name="DecimalCol", type=ossisTypes.DECIMAL),
        FlatColumn(name="DoubleCol", type=ossisTypes.DOUBLE),
        FlatColumn(name="TimestampCol", type=ossisTypes.TIMESTAMP),
    ]
...
def test_unfakeable_types():
    # Create a schema with an unfakeable type (ossisTypes.STRUCT)
    columns = [
        FlatColumn(name="StructCol", type=ossisTypes.STRUCT),
    ]
