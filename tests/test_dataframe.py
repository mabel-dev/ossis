...
def create_schema():
    return RelationSchema(
        name="dataset",
        columns=[
            FlatColumn(name="A", type=ossisTypes.INTEGER, nullable=False),
            FlatColumn(name="B", type=ossisTypes.VARCHAR),
            FlatColumn(name="C", type=ossisTypes.DOUBLE, nullable=False),
        ],
    )
...
def test_to_arrow_struct_column_with_schema():
    schema = RelationSchema(
        ...
        columns=[
            ...
            FlatColumn(
                name="details",
                type=ossisTypes.STRUCT,
                fields=[
                    FlatColumn(name="name", type=ossisTypes.VARCHAR),
                    FlatColumn(name="age", type=ossisTypes.INTEGER),
                ],
            ),
        ],
    )
...
def test_appending():
    df = ossis.DataFrame(schema=cities.schema)
...
def test_to_map_and_to_dict():
    df = ossis.DataFrame(cities.values)
    for row in df:
...
def test_adding_dicts_in_wrong_order():
    df = ossis.DataFrame(
        schema=RelationSchema(
            name="test",
            columns=[
                FlatColumn(name="column_1", type=ossisTypes.INTEGER),
                FlatColumn(name="column_2", type=ossisTypes.VARCHAR),
            ],
        )
    )
